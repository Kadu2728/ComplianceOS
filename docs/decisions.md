...
| D11 | Transactional e-mail provider | DEFAULT: `EmailSender` abstraction; console sender in dev/test; production provider PENDING | HIGH | Auth (reset, invites) |
| D12 | Storage/database provider and region; international transfer analysis | PARTIAL: dev/test use embedded PostgreSQL 16 (`pgserver`, no Docker); production DB/storage/region PENDING + legal review (Res. 19/2024, sources-v1.md B4) | HIGH | Evidence; beta |
