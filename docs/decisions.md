...
| D13 | Compliance OS's own terms and privacy documents | DECIDED (2026-09-19): owner-written Termos de Uso (21 sections) and Política de Privacidade (22 sections) published at `/termos` and `/privacidade` (`apps/web/src/lib/marketing/legal.ts`, verbatim), responsible person Carlos Eduardo Diogo, contact complianceos1199@gmail.com; signup shows the acceptance line; footer Legal column live. Not reviewed by a lawyer — owner's decision; D12 (international transfer) still to be formalized | HIGH | Beta |
| D14 | Text-safe variants of semantic colors; Text Muted usage | DEFAULT (values in docs/design/tokens.md §2) | HIGH | Design tokens |
| D15 | Logo asset | RESOLVED (2026-09-19): owner-provided symbol (PNG, Off-white on transparent) at `apps/web/public/brand/symbol.png`, coloured via CSS mask (`BrandSymbol`); favicon, apple icon and OG image derived from it. SVG still welcome | HIGH | Favicon; marketing |
| D16 | Dark mode in MVP | DEFAULT: no | MEDIUM | Design system |
| D17 | API host | DECIDED (2026-09-17, amended twice): **A (default, live): API and web as two Vercel Hobby projects in `gru1` (Python runtime for FastAPI, `MIGRATE_ON_STARTUP`, `TRUST_PROXY_HEADERS`) + Neon Free `sa-east-1` + Backblaze B2 + free SMTP tier + GitHub Actions daily job** — no card, data in Brazil. **B (paid containers): Docker image + Render blueprint** (`render.yaml`). Koyeb tried and dropped (did not work for the user) | MEDIUM | Deploy |
...
| D25 | Beta billing | DEFAULT: manual — Kiwify checkouts live since 2026-09-26, reconciled by e-mail by hand (no webhook) | LOW | Beta |
...
| D37 | Public landing page | DECIDED (2026-09-19, user request): served at `/` to anonymous visitors by a middleware rewrite to `(marketing)/inicio` (no app route moved); copy from Marketing with the Compliance Researcher's rewrites; showcase = real components on an illustrative fixture; `site.ts` gates contact/social/legal output; interim privacy/terms pages hidden until `legalReady` (razão social); self-serve CTA kept, **publication gated by D13 + human legal review** | HIGH | Marketing; D13 |
| D38 | Pricing and plans | DECIDED (2026-09-19, prices by the owner; matrix by the Product Strategist): first month free with everything from Ultimate, no card; then, per organization per month, **Standard R$ 39** (1 org, 2 users, no Room), **Plus R$ 59** (5 users, Compliance Room with 5 active links, score in the Room — recommended), **Ultimate R$ 89** (15 users, 20 links, Executive summary, e-mail reminders when D11 is on). The core loop (risks, controls, actions, evidence, documents) is never capped. Since 2026-09-26 each tier's CTA "Assinar {plano}" goes to its owner-provided Kiwify checkout (recurring R$ 39/59/89 per month, verified on the checkout pages; **no Kiwify trial**, so it charges on purchase); the free month is the account itself, linked under every tier (`/criar-conta?plano=…`). No webhook: paying changes nothing in the product (everything is open to every organization), and a purchase is matched to its organization by the buyer's e-mail, by hand (D25). Enforcement, trial/read-only mode and Kiwify webhook are backlog | HIGH | Monetization |
...
- **Class:** HIGH
- **Decision:** the owner supplied the approved symbol (three layered open C's) as a raster file; it lives in `apps/web/public/brand/symbol.png` as an Off-white-on-transparent alpha mask and is coloured with `currentColor` through `BrandSymbol` (`components/ui/brand-symbol.tsx`), so one asset serves both themes. The app wordmark fills its reserved 24px slot; `src/app/icon.png`, `apple-icon.png` and `opengraph-image.png` are derived from it. A vector version remains desirable for print/large sizes; nothing in the product depends on it.
...
- **Second amendment (same day, user: "o Koyeb não funciona"):** the API moved to **Vercel's
  Python runtime** as a second Hobby project (`compliance-os-api`, root `apps/api`,
  `[tool.vercel] entrypoint = "app.main:app"`, region `gru1`), next to the web project
  (`compliance-os-web`). Two code changes made this correct: `TRUST_PROXY_HEADERS` (client IP from
  `X-Forwarded-For` for the per-IP limits — behind Vercel the socket peer is the platform) and
  `MIGRATE_ON_STARTUP` (`core/bootstrap.py`: migrations + content seed on cold start, serialized by
  a PostgreSQL advisory lock — serverless hosts have no entrypoint). Hobby constraints accepted for
  the beta: 4.5 MB bodies (`EVIDENCE_MAX_BYTES=4000000`), per-instance limiter, non-commercial
  terms until Pro. With Neon in `sa-east-1`, API, web and data all run in São Paulo — which also
  simplifies the D12 analysis (no international transfer for the primary data). The Docker image and
  the Render blueprint remain the container path. Both Vercel projects were created and deployed
  from the CLI by the Orchestrator; secrets (`DATABASE_URL`, SMTP, S3) are typed by the user.
...
- **Class:** LOW
- **Update (2026-09-26):** the owner published three Kiwify checkouts (Standard `adfcJVQ`, Plus `LzcnU7s`, Ultimate `dOnCkbF`), linked from the landing plans (D38). Still manual: there is no webhook and no plan entity, so the owner matches each Kiwify sale to an organization by the buyer's e-mail (the landing and FAQ ask buyers to use the same e-mail as their account) and cancellations happen in Kiwify.
...
## D37 — Public landing page

- **Status:** DECIDED (2026-09-19, user request)
- **Class:** HIGH — first public commercial surface
- **Process:** Product Strategist (conversion, ICP, 13 blocks) → Marketing & Growth (copy, SEO, JSON-LD) ∥ UX/UI Engineer (visual system, showcase, motion, responsive) → Compliance Researcher (64 passages: 1 veto, 5 rewrites, interim notices) → Senior Software Engineer → QA & Security (SHIP WITH RESERVATIONS; P2 copy fixes applied by the Orchestrator).
- **Where it lives:** `src/middleware.ts` rewrites `/` to `/inicio` when the `cos_access` cookie is absent; authenticated users keep the Visão geral at `/`. Nothing in `(app)` changed route. Geist (display) is loaded only by the marketing layout; six display tokens and `.theme-dark`/`.theme-light` scopes in `globals.css` (tokens.md §3/§6).
- **Truthfulness:** copy uses interface names only, numbers from the repository, limits visible (score = maturity indicator, room caveat, "beta, declared"); showcases render real components (`ScoreCard`, `RadarPanel`, `PrioritiesPanel`, `RiskTable`, `RoomView`…) on a fixture coherent with `score.py` (test-enforced) and captioned as fictional. No customers, certifications, AI, prices or legal claims.
- **Configuration:** `lib/marketing/site.ts` — `contactEmail` (complianceos1199@gmail.com), `instagramUrl` (https://www.instagram.com/compliance_os/), `linkedinUrl` (null), `legalEntity` (null → `legalReady` false: `/privacidade` and `/termos` answer 404 and the footer hides the Legal column). No placeholder can reach the HTML.
- **Publication gates (not commit gates):** D13 (definitive terms/privacy) **and** human legal review of the copy, FAQ and the interim notices (scope in the Researcher's report: residency/D12, score bands F5, room copy, "passaporte" claim); D11 (SMTP) strongly recommended so password recovery works; razão social for the interim notices; Vercel Hobby non-commercial terms before charging. Alternative CTA "Pedir acesso ao beta" is written for the pre-D13 case.
- **Follow-ups (QA P3):** add the three uncovered org-scoped routes to `test_tenancy.py` `ROUTES` (live probes answered 404); `favicon.ico` direct request; `Wordmark` prefetch on auth pages; hover contrast of tertiary links on light sections; `globals-theme` test CRLF robustness.
...
| 2026-09-17 | Phase 14 — LIVE on Vercel (`gru1`) + Neon (`sa-east-1`), beta mode | D17 decided: Render (API + reminders cron + PostgreSQL 16, `render.yaml`, Ohio) and Vercel (`apps/web`, `gru1`). `apps/api/Dockerfile` (uv, no dev deps, non-root, healthcheck) + `docker-entrypoint.sh` (migrate → seed content → uvicorn behind proxy headers; arguments run a job). Settings: `postgres://` URLs normalized to the psycopg driver; production refuses local storage or a missing database URL; HSTS in production. CI gains an `image` job (build, boot against PostgreSQL, run the job, prove the production guard). `docs/deploy.md` runbook with the residency note (Fly `gru` + Neon `sa-east-1` alternative). Local gates PASS (config tests +2). The Docker image itself is only verifiable in CI (no Docker on this machine) — the push is the validation. CI on GitHub proved the image (first run caught the health path: `/api/v1/health`). Same day: Render turned out paid → D17 amended with the zero-cost topology (Koyeb + Neon + B2 + SMTP free tier + GitHub Actions digest workflow). `https://compliance-os-api.onrender.com` identified as an unrelated project. Koyeb dropped (did not work); API moved to Vercel's Python runtime (`TRUST_PROXY_HEADERS`, `MIGRATE_ON_STARTUP` + `core/bootstrap.py`, tests +2). Live: `https://compliance-os-api.vercel.app` (health 200, `gru1`) and `https://compliance-os-web-gilt.vercel.app` (CSP + HSTS, `gru1`); all non-secret production variables and a generated `JWT_SECRET` set from the CLI. Neon provisioned through the Vercel Marketplace (user accepted the terms; `free_v3`, `gru1`; `DATABASE_URL`/`DATABASE_URL_UNPOOLED` injected, unpooled preferred). Beta mode added and tested (`BETA_NO_EMAIL_NO_FILES`: `disabled` e-mail/storage providers answer 503 with pt-BR messages). Production smoke test on the live app: signup → short diagnostic → score 44 → recommendation → one-step plan (201) → radar → audit trail; invitation 503 `email_disabled`; upload 503 `storage_disabled`; note evidence 201; `/api/docs` 404; migrations logged on cold start. Test organization “Teste Deploy Ltda.” (`teste-deploy@complianceos.app`) left in the production database for the user to inspect or delete. Still pending: SMTP + bucket (to leave beta mode), GitHub secrets for the digest, domain (D3), D11/D12/D13. |
| 2026-09-19 | Phase 15 — Landing page (commit) | D37 decided and implemented; D15 resolved. `(marketing)/{inicio,privacidade,termos}`, 19 marketing components, `lib/marketing/{site,entry,copy,fixtures,json-ld}`, `robots.ts`, `sitemap.ts`, icons/OG, symbol in the app wordmark; minimal app refactors (`PrioritiesPanel`, `PlanSummary`, `titleTag` on `PageHeader`/`RoomView`). Gates: lint, typecheck, vitest 35/35 (22 new), `next build` clean — `/inicio` 111 kB First Load JS, shared 103 kB and app routes unchanged; CSP 117/117 scripts nonced; middleware leaves `/api/v1` untouched; regression with the embedded API and a real session (Visão geral, `/riscos/[id]`, `/sala/previa`, logout → landing). Browser: 1280 and 375, drawer, reduced-motion CSS scoping, no overflow. Docs: `deploy.md` residency sentence corrected (D12 pending), `tokens.md` §3/§6. Not published: pending D13 + human legal review (see D37). |
| 2026-09-19 | D13 — legal documents live | Owner-authored Termos de Uso and Política de Privacidade added verbatim (`lib/marketing/legal.ts`, `LegalDocumentPage`), `site.legalEntity` set → `legalReady`; footer Legal column, FAQ 7 link and signup acceptance line render; sitemap lists both pages. Gates: lint, typecheck, vitest 35/35, build (`/termos` and `/privacidade` 110 kB First Load). Browser-verified locally with the embedded API. Human legal review still not done (owner's decision); reminders kept for razão social/CNPJ, D12 and D11. |
| 2026-09-19 | D38 — plans section | Landing block "Planos" rebuilt: trial band + Standard/Plus/Ultimate cards + contact; FAQ 6 rewritten, new FAQ on subscription/cancellation, FAQ 10 with users per plan; hero/final CTA microcopy "Primeiro mês grátis"; JSON-LD with three real BRL offers (no zero-price offer). Gates: lint, typecheck, vitest 35/35, build. Backlog (08-plans.md §9): plan/subscription entity → intent capture → limit enforcement → 30-day trial + read-only mode → D11 → Kiwify webhook → Termos update → monetization metrics. Do not start charging before D11 (invitations) and the Termos trial/read-only clauses. |
| 2026-09-26 | D38/D25 — Kiwify checkouts linked | Tier CTAs "Assinar Standard/Plus/Ultimate" → Kiwify checkouts (external `<a>`, `CtaLink` now treats `http(s)` as external); "Ou comece com 1 mês grátis" under each tier → `/criar-conta?plano=…`; trial band, footnote, FAQ 6 and the subscription FAQ rewritten (charge starts on purchase, same e-mail as the account, cancellation via Kiwify or e-mail — coherent with Termos §6–§7); JSON-LD offers gain `url` and drop the free-month wording. Checkout pages checked read-only: R$ 39/59/89 per month, no trial configured, product name "Compliance OS" on all three. Gates: lint, typecheck, vitest 38/38 (new `marketing-plans.test.ts`), build (`/inicio` 111 kB). Open: Kiwify post-purchase redirect to `/criar-conta` (owner's Kiwify settings), plan names on the Kiwify products, D11 before paying customers need password recovery. |
