...
**Desktop (≥ lg):** left sidebar 248px, `--color-surface-elevated` background, 1px right border. Top of sidebar: wordmark (see §3) at 24px height inside a 56px row. Below: nav list (item height 40px, padding 8/12, icon 20px + label `--text-body-sm` 500 when active / 400 inactive, active indicator 2px `--color-electric-blue` on the left edge, hover `#EBEBE7`). Bottom of sidebar: organization name (`--text-body-sm`, 500) with `Building2` icon and user menu (`CircleUser`) — one client component. Collapsed state: 64px, icons only with `title`; persisted in `localStorage` (convenience only).
