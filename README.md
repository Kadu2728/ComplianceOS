...
### Compliance Agent (language model, optional)

The agent answers eight canonical questions deterministically from the organization's records, with no model involved. Set `LLM_PROVIDER=anthropic` and `ANTHROPIC_API_KEY` (decision D35) to enable free-text questions on Visão geral and "Entender este risco" on a risk page: the model receives only the minimized context bundle, must answer as JSON with the records it cites, and every answer is checked (refs must exist, forbidden legal claims are rejected) and audited. Costs are bounded by `LLM_USER_MINUTE_LIMIT` / `LLM_ORG_HOURLY_LIMIT`. Rules and data boundary: `docs/ai.md`.
...
Web and API as two Vercel projects in `gru1` (`apps/web`; `apps/api` on the Python runtime with `MIGRATE_ON_STARTUP`), PostgreSQL on Neon (`sa-east-1`), files on any S3-compatible bucket (Backblaze B2 for the free path), e-mail through any SMTP relay, the daily digest as a GitHub Actions schedule (`.github/workflows/reminders.yml`). Container alternative: `apps/api/Dockerfile` + `render.yaml`. Step by step, environment variables and the data-residency note: `docs/deploy.md` (decision D17). CI builds and boots the production image on every push.
...
- Both services send hardening headers (API: `nosniff`, `X-Frame-Options: DENY`, `no-referrer`, `default-src 'none'`, `Cache-Control: no-store`; web: a per-request CSP from `src/middleware.ts` with a script nonce and `'strict-dynamic'` — every script Next emits carries the nonce, so an injected inline script does not run — plus `frame-ancestors 'none'`, `form-action 'self'`, `object-src 'none'`, `nosniff`, `Referrer-Policy`, `Permissions-Policy`). `'unsafe-eval'` is added only by `next dev`. Blocking was verified by header and nonce inspection, not yet in a regular browser's console (the in-app test browser does not enforce CSP).
