...
| D38 | Pricing and plans | DECIDED (2026-09-19, prices by the owner; matrix by the Product Strategist): first month free with everything from Ultimate, no card; then, per organization per month, **Standard R$ 39** (1 org, 2 users, no Room), **Plus R$ 59** (5 users, Compliance Room with 5 active links, score in the Room — recommended), **Ultimate R$ 89** (15 users, 20 links, Executive summary, e-mail reminders when D11 is on). The core loop (risks, controls, actions, evidence, documents) is never capped. No billing exists: every plan CTA creates an account (`/criar-conta?plano=…` records intent); the page states that online subscription is not available yet and nobody is charged. D25 stays manual until Kiwify. Enforcement, trial/read-only mode and Kiwify webhook are backlog | HIGH | Monetization |
